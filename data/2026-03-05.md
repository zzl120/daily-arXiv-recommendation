...
Motivation: 现有多模态序列推荐模型大多依赖启发式融合策略，无法捕捉用户-模态交互的动态性和上下文敏感性。用户对模态的偏好不仅因人而异，同一用户在不同物品或类别间也会变化，且模态间的协同效应尚未得到充分探索。

Method: 提出类别引导注意力专家混合模块，通过辅助类别预测任务动态分配模态权重；设计模态交换对比学习任务增强跨模态表示对齐；整合ID、文本和视觉三种模态信息，使用Transformer编码用户兴趣序列。

Result: 在四个公开数据集上的实验表明，CAMMSR在NDCG@10和MRR@10等指标上显著优于现有最先进方法，相对改进最高达15.32%。消融研究验证了各组件的重要性，效率分析显示模型具有竞争力的训练和推理效率。
